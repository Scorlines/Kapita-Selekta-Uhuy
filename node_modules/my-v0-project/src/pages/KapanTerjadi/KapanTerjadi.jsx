...
import { useReport } from '../../context/ReportContext'; // ← TAMBAH INI
...
function KapanTerjadi() {
  ...
  const { reportData, updateMultipleFields } = useReport(); // ← TAMBAH INI
  
  // Ambil dari Context atau sessionStorage sebagai fallback
  const [selectedDate, setSelectedDate] = useState(reportData.date || '');
  const [selectedLocation, setSelectedLocation] = useState(reportData.location || '');
  const [customLocation, setCustomLocation] = useState(reportData.locationDetail || '');
  const bullyingType = reportData.type || sessionStorage.getItem('bullyingType') || '';
  ...
  const handleSubmit = () => {
    ...
    // Simpan tanggal dan lokasi ke Context (untuk database)
    updateMultipleFields({
      date: selectedDate,
      location: finalLocation,
      locationDetail: customLocation.trim()
    }); // ← TAMBAH INI
    
    // Tetap simpan ke sessionStorage juga sebagai backup
    sessionStorage.setItem('tanggalKejadian', selectedDate);
    sessionStorage.setItem('lokasiKejadian', finalLocation);
    
    console.log('Date & Location saved to context:', { // ← DEBUG
      date: selectedDate,
      location: finalLocation
    });
