...
function Response5() {
  ...
  const { reportData, updateReportData } = useReport(); // ← TAMBAH INI
  
  // Ambil dari Context atau sessionStorage sebagai fallback
  const [storyDetail, setStoryDetail] = useState(reportData.description || '');
  const bullyingType = reportData.type || sessionStorage.getItem('bullyingType') || 'Aku diejek teman';
  ...
  const handleKirim = () => {
    ...
    // Simpan detail cerita ke Context (untuk database)
    updateReportData('description', storyDetail); // ← TAMBAH INI
    
    // Tetap simpan ke sessionStorage juga sebagai backup
    sessionStorage.setItem('storyDetail', storyDetail);
    
    console.log('Story saved to context:', storyDetail); // ← DEBUG
